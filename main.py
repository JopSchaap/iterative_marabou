...
def plot_response(net: Marabou.MarabouNetwork, axis = [0,1], out_axis=None, min_value=-1.0, max_value=1.0, nSamples=(30, 30), base = None, box=None):
    ...
    for i, x in enumerate(tqdm(np.linspace(min_value, max_value, nSamples[0]))):
        for j, y in enumerate(np.linspace(min_value, max_value, nSamples[1])):
            ...
            extra[i,j] = 1 if res[out_axis[0]] < 0.8 else 0
            result[:,i,j] = res[out_axis]

    for i in trange(result.shape[0], delay=1):
        plt.figure()
...
def plot_uncertainty(optimistic_sizes: np.ndarray, pessimistic_sizes:np.ndarray):
    ...
    # plt.yscale("log")
    plt.xlabel("iteration")
    plt.ylabel("difference between optimistic and pessimistic")
    saveFig()
...
if __name__ == "__main__":
    ...
    plot_time_per_iteration(iterator.time_list)
